...
void GetPose::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
    model = _parent;
    ...
    else ROS_INFO("GetPose plugin missing <robotNamespace>, defaults to \"%s\"", robot_namespace_.c_str());

    this->pose_topic_ = this->model->GetName() + "/get_pose";
    if (!_sdf->HasElement("poseTopic"))
    {
        ROS_WARN("GetPose plugin (ns = %s) missing <poseTopic>, defaults to \"%s\"", robot_namespace_.c_str(), this->pose_topic_.c_str());
    }
...
GZ_REGISTER_MODEL_PLUGIN(GetPose);
